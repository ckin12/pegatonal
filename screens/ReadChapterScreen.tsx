...
// ... (phần import giữ nguyên như bạn gửi)
...
const ReadChapterScreen = () => {
  ...
  const { comicId, comicTitle, chapterNumber } = route.params as {
    comicId: string;
    comicTitle: string;
  ...
  const handleCommentSubmit = () => {
    ...
    updated[currentChapter].push({ text: newComment.trim(), time: new Date().toLocaleTimeString() });
    setComments(updated);
    setNewComment('');
  ...
  return (
    <View style={styles.container}>
      {loading && <View style={styles.loadingOverlay}><ActivityIndicator size="large" color="#3b82f6" /></View>}
      ...
      >
        <Text style={styles.title}>{`${comicTitle} - ${chapter.title}`}</Text>
  ...
  >
    ...
    />
        ...
        {/* Like, Follow, Info buttons */}
        <View style={styles.actionsRow}>
          <TouchableOpacity
            style={[styles.likeButton, { backgroundColor: isLiked ? '#F25C05' : '#fff' }]}
            onPress={handleLike}
          >
            <FontAwesome name="thumbs-up" size={18} color={isLiked ? '#fff' : '#007AFF'} style={{ marginRight: 6 }} />
            <Text style={{ color: isLiked ? '#fff' : '#000', fontWeight: 'bold' }}>Thích ({likesCount})</Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={() => Alert.alert('Thông tin truyện', `Truyện: ${comicTitle}\nID: ${comicId}`)}>
            <Text style={styles.authorText}>Truyện: <Text style={styles.authorName}>{comicTitle}</Text></Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.followButton, { backgroundColor: isFollowed ? '#F25C05' : '#fff' }]}
            onPress={() => setIsFollowed(!isFollowed)}
          >
            <FontAwesome name="user" size={18} color={isFollowed ? '#fff' : '#007AFF'} style={{ marginRight: 6 }} />
            <Text style={{ color: isFollowed ? '#fff' : '#000', fontWeight: 'bold' }}>{isFollowed ? 'Đã theo dõi' : 'Theo dõi'}</Text>
          </TouchableOpacity>
        </View>
{/* Nút tải chương */}
<View style={{ marginTop: 20, alignItems: 'center' }}>
          <TouchableOpacity
            onPress={() => navigation.navigate('SelectChapter')}
            style={{
          ...
          >
            <Text style={{ color: '#fff', fontWeight: 'bold' }}>⬇ Tải chương</Text>
          </TouchableOpacity>
        </View>
        {/* Comment section */}
        <Text style={styles.commentTitle}>Bình luận</Text>
        {comments[currentChapter].map((c, i) => (
          <View key={i} style={styles.commentItemRow}>
            ...
            <View style={styles.commentContent}>
              <View style={styles.commentHeader}>
                ...
                <View style={styles.memberTag}><Text style={styles.memberTagText}>Thành viên</Text></View>
                <Text style={styles.commentTime}>• {c.time}</Text>
              </View>
              <Text style={styles.commentText}>{c.text}</Text>
              <View style={styles.commentActions}>
                <TouchableOpacity onPress={() => openEditCommentModal(i)} style={styles.smallButton}><Text style={{ color: '#007AFF' }}>Sửa</Text></TouchableOpacity>
                <TouchableOpacity onPress={() => handleDeleteComment(i)} style={styles.smallButton}><Text style={{ color: 'red' }}>Xóa</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        ))}
      ...
      {/* ✅ Thanh điều hướng chương quay lại đầy đủ */}
      {showNavBar && (
        ...
        <View style={styles.modalBackground}>
          <View style={styles.modalContent}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 10 }}>
              <TouchableOpacity onPress={() => setEditModalVisible(false)}><Text style={{ color: 'red' }}>Hủy</Text></TouchableOpacity>
              <TouchableOpacity onPress={confirmEditComment}><Text style={{ color: '#007AFF' }}>Lưu</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
